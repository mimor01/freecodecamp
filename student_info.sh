...
echo "$($PSQL "SELECT DISTINCT(course) FROM students FULL JOIN majors USING(major_id) FULL JOIN majors_courses USING(major_id) FULL JOIN courses USING(course_id) WHERE students.student_id IS NULL OR (first_name ='Obie' AND last_name='Hilpert') ORDER BY course DESC")"
